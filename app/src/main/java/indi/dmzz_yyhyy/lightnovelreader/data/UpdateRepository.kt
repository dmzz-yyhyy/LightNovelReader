...
class UpdateRepository @Inject constructor(
        ...
        private fun deleteSingleFile(file: File, isThrowFileNotFoundException: Boolean = true): Boolean {
                ...
                return if (file.exists() && file.isFile) {
                        ...
                        if (isThrowFileNotFoundException) { Log.e("DeleteFile", "${file.path} not found") }
                        false
                }
        }
        private fun writeResponseBodyToDisk(body: ResponseBody): Boolean {
                return try {
                        ...
                        try {
                                ...
                                outputStream.flush()
                                Log.d("FileDownload", "file saved at" + Environment.getExternalStorageDirectory().path + "/download/" + "LightNovelReaderUpdate.apk")
                                true
                        } catch (e: IOException) {
        ...
        suspend fun updateApp(context: Context) {
                deleteSingleFile(File(Environment.getExternalStorageDirectory().path + "/download/" + "LightNovelReaderUpdate.apk"), false)
                if (saveUpdateApk()) {
                        val intent = Intent(Intent.ACTION_VIEW)
                        val apkFile = File(Environment.getExternalStorageDirectory().path + "/download/" + "LightNovelReaderUpdate.apk")
