...
fun ExpandedPageScreen(
    ...
    LaunchedEffect(expandedPageUiState.isLoading) {
        if (!expandedPageUiState.isLoading) isRefreshing = false
    }
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            modifier = Modifier
                ...
                .fillMaxSize(),
            isRefreshing = isRefreshing && expandedPageUiState.isLoading,
            ...
            LazyColumn(
                ...
                item {
                    AnimatedVisibility(
                        visible = expandedPageUiState.isLoading && !isRefreshing,
                ...
                if (expandedPageUiState.errorMessage != null) {
                    item {
                        EmptyPage(
                            modifier = Modifier.fillMaxWidth().padding(vertical = 24.dp),
                            icon = painterResource(R.drawable.error_24px),
                            title = stringResource(R.string.related_books_load_failed),
                            description = expandedPageUiState.errorMessage.orEmpty()
                        ) {
                            TextButton(onClick = refreshResult) {
                                Text(stringResource(R.string.related_books_retry))
                            }
                        }
                    }
                } else if (expandedPageUiState.isComplete && expandedPageUiState.bookList.isEmpty()) {
                    item {
                        EmptyPage(
                            modifier = Modifier.fillMaxWidth().padding(vertical = 24.dp),
                            icon = painterResource(R.drawable.not_found_90dp),
                            title = stringResource(R.string.nothing_here),
                            description = stringResource(R.string.related_books_empty)
                        )
                    }
                }
    ...
    LaunchedEffect(listState) {
        snapshotFlow {
            ...
            (layoutInfo.visibleItemsInfo.lastOrNull()?.index ?: -1) to expandedPageUiState.bookList.size
        }.collect { (lastVisibleIndex, size) ->
