...
class Wenku8HomeExploreTapPage(
    ...
    private fun getBooksRow(index: Int, soup: Document?): ExploreBooksRow {
        val title =
            soup?.selectFirst("#centers > div:nth-child(${index + 2}) > div.blocktitle")?.text()
                ?.split("(")?.getOrNull(0) ?: ""
        val idlList =
            soup?.select("#centers > div:nth-child(${index + 2}) > div.blockcontent > div > div > a:nth-child(1)")
                ?.map { it.attr("href").replace("/book/", "").replace(".htm", "") }
        val titleList =
            soup?.select("#centers > div:nth-child(${index + 2}) > div.blockcontent > div > div > a:nth-child(3)")
                ?.map { it.text().split("(").getOrNull(0) ?: "" } ?: emptyList()
        val coverUrlList =
            soup?.select("#centers > div:nth-child(${index + 2}) > div.blockcontent > div > div > a:nth-child(1) > img")
