...
class TextProcessingRepository @Inject constructor(
    ...
    fun processBookInformation(block: () -> BookInformation): BookInformation =
        process(block.invoke()) { it::processBookInformation }

    suspend fun coroutineProcessBookInformation(block: suspend () -> BookInformation): BookInformation =
    ...
    fun processBookVolumes(block: () -> BookVolumes): BookVolumes =
        process(block.invoke()) { it::processBookVolumes }
    ...
    suspend fun coroutineProcessChapterContent(
        bookId: String,
        block: suspend () -> ChapterContent
    ): ChapterContent = process(block.invoke()) { processor ->
        {
            processor.processChapterContent(
                bookId, it, ComponentProcessor(
                    contentComponentRepository.serializeMap,
    ...
    fun processExploreBooksRow(exploreDisplayBook: ExploreDisplayBook): ExploreDisplayBook =
        process(exploreDisplayBook) { it::processExploreBooksRow }

    init {
        registerProcessors(
            "simplified_traditional_processor".ofId(),
            simplifiedTraditionalProcessor
        )
